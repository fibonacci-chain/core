...
func TestOrganizeAggregate(t *testing.T) {
	...
	sdrBallot := types.ExchangeRateBallot{
		types.NewVoteForTally(sdk.NewDec(17), utils.MicroFiboDenom, ValAddrs[0], power),
		types.NewVoteForTally(sdk.NewDec(10), utils.MicroFiboDenom, ValAddrs[1], power),
		types.NewVoteForTally(sdk.NewDec(6), utils.MicroFiboDenom, ValAddrs[2], power),
	}
	...
	sort.Sort(sdrBallot)
	sort.Sort(krwBallot)
	sort.Sort(ballotMap[utils.MicroFiboDenom])
	sort.Sort(ballotMap[utils.MicroAtomDenom])

	require.Equal(t, sdrBallot, ballotMap[utils.MicroFiboDenom])
