...
func (suite *KeeperTestSuite) CommitAfter(t time.Duration) {
	...
	suite.app.EndBlock(suite.ctx, abci.RequestEndBlock{Height: header.Height})
	suite.app.Commit(suite.ctx.Context())

	header.Height += 1
	header.Time = header.Time.Add(t)
	suite.app.BeginBlock(suite.ctx, abci.RequestBeginBlock{
		Header: header,
	})
