...
func cosmosAddressFromArg(addr string) (sdk.AccAddress, error) {
	...
	return types.AccAddressFromHexUnsafe(addr)
}
...
func TestCosmosToEthereumTypes(t *testing.T) {
	...
	cosmosAddr, err := types.AccAddressFromHexUnsafe(hexString[2:])
...
func TestAddressToCosmosAddress(t *testing.T) {
	baseAddr, err := types.AccAddressFromHexUnsafe("6A98D72760f7bbA69d62Ed6F48278451251948E7")
	require.NoError(t, err)
