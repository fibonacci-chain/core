...
# Testing whether fbchain works or not
fbchaind version # Uncomment the below line if there are any dependency issues
# ldd build/fbchaind

# Initialize validator node
MONIKER="fbchaind-node-$NODE_ID"

fbchaind init "$MONIKER" --chain-id fbchaind_9001-9001 >/dev/null 2>&1
...
SEI_NODE_ID=$(fbchaind tendermint show-node-id)
...
printf "12345678\n12345678\ny\n" | fbchaind keys add "$ACCOUNT_NAME" >/dev/null 2>&1

# Get genesis account info
GENESIS_ACCOUNT_ADDRESS=$(printf "12345678\n" | fbchaind keys show "$ACCOUNT_NAME" -a)
echo "$GENESIS_ACCOUNT_ADDRESS" >> build/generated/genesis_accounts.txt

# Add funds to genesis account
fbchaind add-genesis-account "$GENESIS_ACCOUNT_ADDRESS" 10000000usei

# Create gentx
printf "12345678\n" | fbchaind gentx "$ACCOUNT_NAME" 10000000usei --chain-id sei
...
SEIVALOPER_INFO=$(printf "12345678\n" | fbchaind keys show "$ACCOUNT_NAME" --bech=val -a)
PRIV_KEY=$(printf "12345678\n12345678\n" | fbchaind keys export "$ACCOUNT_NAME")
