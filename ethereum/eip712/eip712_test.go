...
import (
	...
	config "github.com/fibonacci-chain/core/app/params"
...
func (suite *EIP712TestSuite) SetupTest() {
	...
	sdk.GetConfig().SetBech32PrefixForAccount(config.Bech32PrefixAccAddr, "")
...
func (suite *EIP712TestSuite) makeCoins(denom string, amount sdk.Int) sdk.Coins {
	return sdk.NewCoins(
		sdk.NewCoin(
			denom,
			amount,
...
func (suite *EIP712TestSuite) TestEIP712() {
	...
	params := EIP712TestParams{
		fee: txtypes.Fee{
			Amount:   suite.makeCoins(suite.denom, sdk.NewInt(2000)),
			GasLimit: 20000,
		},
	...
	}{
		{
			...
			msgs: []sdk.Msg{
				banktypes.NewMsgSend(
					...
					suite.createTestAddress(),
					suite.makeCoins(suite.denom, sdk.NewInt(1)),
				),
			},
			expectSuccess: true,
		},
		...
		},
		{
			...
			msgs: []sdk.Msg{
				stakingtypes.NewMsgDelegate(
					...
					suite.makeCoins(suite.denom, sdk.NewInt(1))[0],
				),
			},
			expectSuccess: true,
		},
		...
		},
		{
			...
			msgs: []sdk.Msg{
				...
				stakingtypes.NewMsgDelegate(
					params.address,
					sdk.ValAddress(suite.createTestAddress()),
					suite.makeCoins(suite.denom, sdk.NewInt(5))[0],
				),
			},
			expectSuccess: true,
		},
		...
		},
		{
			...
			msgs: []sdk.Msg{
				...
				banktypes.NewMsgSend(
					params.address,
					suite.createTestAddress(),
					suite.makeCoins(suite.denom, sdk.NewInt(50)),
				),
			},
