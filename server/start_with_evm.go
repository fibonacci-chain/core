		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			if flagChainID != "" {
				if flagChainID != chainID {
					panic(fmt.Sprintf("chain-id mismatch: %s vs %s. The chain-id passed in is different from the value in ~/.fbchaind/config/client.toml \n", flagChainID, chainID))
				}
				chainID = flagChainID
			}
...
func startStandAlone(ctx *servercosmos.Context, appCreator types.AppCreator) error {
...
func startInProcess(
	...
	//start json rpc
	logger := ctx.Logger
	serverConfig, err := serverCfg.GetConfig(ctx.Viper)
	if err != nil {
		panic(err)
	}

	var idxer ethermint.EVMTxIndexer
	if serverConfig.JSONRPC.EnableIndexer {
	...
	if serverConfig.JSONRPC.Enable {
		...
		httpSrv, httpSrvDone, err = StartJSONRPC(ctx, clientCtx, tmRPCAddr, tmEndpoint, &serverConfig, idxer)
		if err != nil {
			return err
		}
