...
def add_key(account_name, local=False):
    if local:
        add_key_cmd = f"yes | ~/go/bin/fbchaind keys add {account_name} --keyring-backend test"
    else:
        add_key_cmd = f"printf '12345678\n' | ~/go/bin/fbchaind keys add {account_name}"
    ...
    address = splitted_outputs[1].split(': ')[1]
...
def add_account(account_name, address, mnemonic, local=False):
    if local:
        add_account_cmd = f"~/go/bin/fbchaind add-genesis-account {address} 1000000000ufibo --keyring-backend test"
    else:
        add_account_cmd = f"printf '12345678\n' | ~/go/bin/fbchaind add-genesis-account {address} 1000000000ufibo"
