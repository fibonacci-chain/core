    ...
    while i < len(validators):
        ...
        while j < num_concurrent:
            ...
            subprocess.check_output(
                f"ssh -i {ssh_key} ubuntu@{validators[i + j]} 'sudo -S -p \"\" systemctl restart fbchaind'")
            j += 1
        i += num_concurrent
...
def delegation_change(chain_id):
    ...
    fbchaind_query_cmd = f"fbchaind q staking delegations {admin_acc} --chain-id {chain_id} --output json"
    staking_output = json.loads(
        subprocess.check_output([fbchaind_query_cmd], stderr=subprocess.STDOUT,
                                shell=True))
    total_delegation = 0
    validator_addrs = []
    ...
    for i in range(len(validator_addrs // 4)):
        ...
        fbchaind_staking_cmd = f"fbchaind tx staking delegate {validator_addrs[i]} {10 * total_delegation}ufibo --from admin --chain-id {chain_id} -b block -y"
        _run_fbchaind_cmd(fbchaind_staking_cmd)
    time.sleep(random.randint(600, 3600))
    # Unbond
    for validator in delegations:
        print(f"Unbonding {10 * total_delegation} from {validator}")
        fbchaind_unbond_cmd = f"fbchaind tx staking unbond {validator} {delegations[validator]}ufibo --from admin -b block -y --chain-id {chain_id}"
        _run_fbchaind_cmd(fbchaind_unbond_cmd)
...
def _get_admin_acc():
    fbchaind_query_cmd = "fbchaind keys list --output json"
    accs_output = _run_fbchaind_cmd(fbchaind_query_cmd)
    return filter(lambda x: x['name'] == 'admin', accs_output)['address']

def _run_fbchaind_cmd(cmd):
    fbchaind_cmd = "printf '12345678\n' " + cmd
    output = json.loads(
        subprocess.check_output([fbchaind_cmd], stderr=subprocess.STDOUT,
                                shell=True))
    return output
