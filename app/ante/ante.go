...
// transaction-level processing (e.g. fee payment, signature verification) before
// being passed onto it's respective handler.
func NewAnteHandler2(options ante2.HandlerOptions) (sdk.AnteDepGenerator, error) {
	return func(txDeps []sdkacltypes.AccessOperation, tx sdk.Tx, txIndex int) (newTxDeps []sdkacltypes.AccessOperation, err error) {
		...
		if ok {
			...
			if len(opts) > 0 {
				...
				return anteDepGenerator(txDeps, tx, txIndex)
			}
		}
		...
		default:
			return nil, errorsmod.Wrapf(errortypes.ErrUnknownRequest, "invalid transaction type: %T", tx)
		}
