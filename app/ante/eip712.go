...
func NewLegacyCosmosAnteHandlerEip712(options HandlerOptions) sdk.AnteHandler {
	...
	a, _ := sdk.ChainAnteDecorators(
		...
		sdk.DefaultWrappedAnteDecorator(authante.NewValidateBasicDecorator()),
		sdk.DefaultWrappedAnteDecorator(authante.NewTxTimeoutHeightDecorator()),
		sdk.DefaultWrappedAnteDecorator(NewMinGasPriceDecorator(options.FeeMarketKeeper, options.EvmKeeper)),
		sdk.DefaultWrappedAnteDecorator(authante.NewValidateMemoDecorator(options.AccountKeeper)),
		sdk.DefaultWrappedAnteDecorator(authante.NewConsumeGasForTxSizeDecorator(options.AccountKeeper)),
		sdk.DefaultWrappedAnteDecorator(authante.NewDeductFeeDecorator(options.AccountKeeper, options.BankKeeper, options.FeegrantKeeper, options.ParamsKeeper, options.TxFeeChecker)),
		// SetPubKeyDecorator must be called before all signature verification decorators
		sdk.DefaultWrappedAnteDecorator(authante.NewSetPubKeyDecorator(options.AccountKeeper)),
		sdk.DefaultWrappedAnteDecorator(authante.NewValidateSigCountDecorator(options.AccountKeeper)),
		sdk.DefaultWrappedAnteDecorator(authante.NewSigGasConsumeDecorator(options.AccountKeeper, options.SigGasConsumer)),
		// Note: signature verification uses EIP instead of the cosmos signature validator
		sdk.DefaultWrappedAnteDecorator(NewLegacyEip712SigVerificationDecorator(options.AccountKeeper, options.SignModeHandler)),
		sdk.DefaultWrappedAnteDecorator(authante.NewIncrementSequenceDecorator(options.AccountKeeper)),
...
func VerifySignature(
	...
	case *signing.SingleSignatureData:
		...
		signerChainID, err := evmtypes.ParseChainID(signerData.ChainID)
		if err != nil {
