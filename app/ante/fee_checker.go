...
func NewDynamicFeeChecker(k DynamicFeeEVMKeeper) authante.TxFeeChecker {
	return func(ctx sdk.Context, tx sdk.Tx, simulate bool, paramsKeeper paramskeeper.Keeper) (sdk.Coins, int64, error) {
		...
		maxPriorityPrice := sdk.NewInt(math.MaxInt64)
		...
		feeCap := fee.Quo(sdk.NewIntFromUint64(gas))
		baseFeeInt := sdk.NewIntFromBigInt(baseFee)
		...
		effectivePrice := sdk.NewIntFromBigInt(types.EffectiveGasPrice(baseFeeInt.BigInt(), feeCap.BigInt(), maxPriorityPrice.BigInt()))

		// NOTE: create a new coins slice without having to validate the denom
		effectiveFee := sdk.Coins{
			{
				Denom:  denom,
				Amount: effectivePrice.Mul(sdk.NewIntFromUint64(gas)),
			},
		}
