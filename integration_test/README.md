...
There's a template yaml file which you can copy from to start with: [template](https://github.com/fibonacci-chain/core/tree/master/integration_test/template/template_test.yaml)
...
- name: Test number of validators should be equal to 4
  inputs:
    ...
    - cmd: fbchaind q tendermint-validator-set |grep address |wc -l
      env: RESULT
  verifiers:
  - type: eval
    expr: RESULT == 4
...
| cmd        | Yes      | Exact fbchaind or bash command to run.                                                                                                            |
