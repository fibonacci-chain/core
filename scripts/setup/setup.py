...
def validate_clean_state():
    ...
    logging.info('Updating fbchaind binary...')
...
def validate_version(version):
    ...
    version_json_output = json.loads(run_command('fbchaind version --long --output json'))
...
def set_price_feeder():
    ...
    addr, _ = fbchaind_add_key(ORACLE_PRICE_FEEDER_ACC_NAME)
    run_with_password(
        f'fbchaind tx oracle set-feeder $(fbchaind keys show {ORACLE_PRICE_FEEDER_ACC_NAME} -a) --from admin --yes --fees=2000usei',
...
def output_price_feeder_config(chain_id):
    ...
    val_addr = json.loads(run_with_password(f'fbchaind keys show {DEFAULT_VALIDATOR_ACC_NAME} --bech=val --output json', key_password))['address']
...
def init_sei(chain_id, moniker):
    ...
    run_command(f'fbchaind init {moniker} --chain-id {chain_id}')
...
def try_fbchaind_delete_key(account_name, key_password):
    try:
        run_with_password(f'fbchaind keys delete {account_name} -y', key_password)
        logging.info("Deleted existing key if it exists.")
    except Exception:
        logging.info("No existing key found.")


def fbchaind_add_key(account_name):
    """Add a key to the SEI blockchain."""
    key_password = getpass(f'Please enter a password for the account={account_name}: \n')
    try_fbchaind_delete_key(account_name, key_password)
    logging.info("Deleted existing key if it exists.")

    add_key_output = run_with_password(f'fbchaind keys add {account_name} --output json', key_password)
...
def add_genesis_account(account_name, starting_balance):
    ...
    run_command(f'fbchaind add-genesis-account {address} {starting_balance}')
    logging.info('Added genesis account %s with address %s', account_name, address)
    return address
...
def gentx(chain_id, account_name, starting_delegation, gentx_args):
    ...
    output = run_with_password(f'fbchaind gentx {account.account_name} {starting_delegation} --chain-id={chain_id} {gentx_args}', account.password)
    logging.info(output)
...
def setup_validator(args):
    ...
    fbchaind_add_key(DEFAULT_VALIDATOR_ACC_NAME)
